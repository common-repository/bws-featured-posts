...
if ( ! function_exists( 'ftrdpsts_init' ) ) {
	function ftrdpsts_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $ftrdpsts_plugin_info, '3.9' );
	}
}
...
if ( ! function_exists( 'ftrdpsts_settings' ) ) {
	function ftrdpsts_settings() {
		...
		if ( ! isset( $ftrdpsts_options['plugin_option_version'] ) || $ftrdpsts_options['plugin_option_version'] != $ftrdpsts_plugin_info["Version"] ) {
			...
			$ftrdpsts_options['display_not_supported_notice'] = 1;
...
if ( ! function_exists( 'ftrdpsts_register_plugin_links' ) ) {
	function ftrdpsts_register_plugin_links( $links, $file ) {
		...
		if ( $file == $base ) {
			...
			$links[] = '<a href="https://support.bestwebsoft.com" target="_blank">' . __( 'FAQ','bws-featured-posts' ) . '</a>';
			$links[] = '<a href="https://support.bestwebsoft.com">' . __( 'Support','bws-featured-posts' ) . '</a>';
		}
		return $links;
	}
}
...
if ( ! function_exists ( 'ftrdpsts_admin_notices' ) ) {
	function ftrdpsts_admin_notices() {
		global $hook_suffix, $ftrdpsts_options;

		$admin_pages = array( 'plugins.php', 'update-core.php' );
		if ( in_array( $hook_suffix, $admin_pages ) || ( isset( $_GET['page'] ) && "featured-posts.php" == $_GET['page'] ) ) {
			if ( empty( $ftrdpsts_options ) )
				ftrdpsts_settings();

			if ( ! $ftrdpsts_options['display_not_supported_notice'] )
				return;

			if ( isset( $_POST['bws_hide_not_supported_notice'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'bws_settings_nonce_name' ) ) {
				$ftrdpsts_options['display_not_supported_notice'] = 0;
				update_option( 'ftrdpsts_options', $ftrdpsts_options );
				return;
			} ?>
			<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
				<div class="bws_banner_on_plugin_page bws_banner_to_settings">
					<div class="icon">
						<img title="" src="<?php echo plugins_url( 'images/featured-posts-icon.png', __FILE__ ); ?>" alt="" />
					</div>
					<div class="text">
						<strong><?php printf( __( '%s becomes %s plugin', 'bestwebsoft' ), 'Featured Posts', 'Relevant' ); ?></strong>
						<br />
						<?php printf( __( '%s plugin is now a part of %s plugin. It will be no longer supported (updates will be unavailable) starting from July 2017. Install %s plugin now to automatically apply your current settings and get new amazing features.', 'bestwebsoft' ), 'Featured Posts', 'Relevant – Related, Featured, Latest, and Popular Posts', 'Relevant' ); ?>
						<br />
